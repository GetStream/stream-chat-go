...
- The target user has at least one device [registered](/chat/docs/go-golang/push_devices/) with the Stream App.

- The target user is a member of the channel that will send notifications.

- The target user has **not** [muted](/chat/docs/go-golang/muting_channels/) the channel the pushed message is from.

- The target user has **not** muted the user that created the pushed message.

- Push notifications are enabled in the channel type.

- If using [thread](/chat/docs/go-golang/threads/), the user is part of the thread that messages are sent in (i.e previously posted at least one message or were mentioned in that thread)
...
At least one device must be registered for a specific user in order to receive push notifications. This is also a push delivery condition and  a mandatory step in the [push integration logic](/chat/docs/go-golang/push_devices/#register-a-device/).
...
This is why we recommend testing your push integration using the [Stream CLI tool](/chat/docs/go-golang/push_test/) first, as it runs some checks such as having a registered device. You can learn more the CLI tool [here](https://github.com/GetStream/chat-push-test/tree/master/react-native).

You can also check if a device is registered by [retrieving the list of devices for a given user](/chat/docs/go-golang/push_devices/#list-devices/).
...
- You will need to **remove this existing registration** token from the Stream (i.e [Remove Device](/chat/docs/go-golang/push_devices/#unregister-a-device/)) and stop using it to send messages.

- **Add a new valid registration token** for that device (i.e [Add Device](/chat/docs/go-golang/push_devices/#register-a-device/)).
