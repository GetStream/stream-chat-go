...
- Your app has push notifications configured for at least one provider [in your app settings](/chat/docs/go-golang/app_setting_overview/)

- You have a user that has at least one [device](/chat/docs/go-golang/#push_devices/) associated
...
```go
skipDevices := true
req := &CheckPushRequest{MessageID: message.ID, SkipDevices: &skipDevices, UserID: user.ID}
...
req := &CheckPushRequest{
 ...
 UserID: user.ID,
 ApnTemplate: "{\"aps\":{\"alert\":{\"title\":\"{{ sender.name }} @ {{ channel.name }}\",\"body\":\"testing out new stuff:{{ message.text }}\"},\"category\":\"NEW_MESSAGE_2\"}}",
}
client.CheckPush(ctx, req)
```
