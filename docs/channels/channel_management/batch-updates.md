...
filtersByCIDS := ChannelsBatchFilters{
...
```go
// Add members
updater := client.ChannelBatchUpdater()
...
resp, err := updater.AddMembers(ctx, filter, members)

// Update channel data
updater := client.ChannelBatchUpdater()
filter := ChannelsBatchFilters{
    Types: map[string]interface{}{
        "$in": []string{"messaging", "team"},
    },
}
...
resp, err := updater.UpdateData(ctx, filter, data)
```
...
```go
taskResponse, err := client.GetTask(context.Background(), response.TaskID)
```
