...
> [!NOTE]
> In legacy permission system users can never access users nor channels from other teams. In [Permissions V2](/chat/docs/go-golang/chat_permission_policies/) it is possible to alter this behavior using multi-tenant permissions.
...
response, err := client.UpsertUsers(ctx, user)
```
...
In tables below you will find default permission grants for builtin roles that designed for multi-tenant applications. They are useful for [multi-tenant applications](/chat/docs/go-golang/multi_tenant_chat/) only.
...
response, err := client.QueryTeamUsageStats(ctx, nil)

// Query specific month
response, err := client.QueryTeamUsageStats(ctx, &QueryTeamUsageStatsRequest{
    Month: "2024-01",
})
...
if response.Next != "" {
    nextPage, err := client.QueryTeamUsageStats(ctx, &QueryTeamUsageStatsRequest{
        Limit: &limit,
        Next:  response.Next,
    })
}
