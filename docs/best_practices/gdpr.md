...
GDPR gives EU citizens the right to request access to their information and the right to have access to this information in a portable format. Stream Chat covers this requirement with the [Export User](/chat/docs/go-golang/exporting_channels/) method.

This method can only be used with server-side authentication.

```go
user, err := serverClient.ExportUser(userID, nil)
```
...
- [Deleting a batch of users](/chat/docs/go-golang/update_users/#deleting-many-users/)

- [Deleting a batch of channels](/chat/docs/go-golang/channel_delete/#deleting-many-channels/)
