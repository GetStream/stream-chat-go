...
filledMessage, err := channel.SendMessage(message, userID, MessageSkipPush)
```
...
> [!WARNING]
> Deleting a message does not delete its file attachments. See [deleting attachments](/chat/docs/go-golang/file_uploads/#deleting-files-and-images/) for more information.
