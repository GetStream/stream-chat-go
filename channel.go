...
type MarkDeliveredOptions struct {
	ChannelDeliveredMessage map[string]string `json:"channel_delivered_message"`
...
func (ch *Channel) MarkDelivered(ctx context.Context, options *MarkDeliveredOptions) (*Response, error) {
	if options == nil {
		return nil, errors.New("options must not be nil")
	}

	if len(options.ChannelDeliveredMessage) == 0 {
		return nil, errors.New("channel_delivered_message must not be empty")
	}
	...
	var resp Response
	err := ch.client.makeRequest(ctx, http.MethodPost, "channels/delivered", params, options, &resp)
	return &resp, err
}

// MarkDeliveredSimple is a convenience method to mark a message as delivered for a specific user.
func (ch *Channel) MarkDeliveredSimple(ctx context.Context, userID string, messageID string) (*Response, error) {
	if userID == "" {
	...
	options := &MarkDeliveredOptions{
		ChannelDeliveredMessage: map[string]string{ch.cid(): messageID},
		UserID:                  userID,
	}

	return ch.MarkDelivered(ctx, options)
}
